...
if [ ! -n "$1" ] ;then
	...
	ls $Top_Path/lichee/linux-4.9/drivers/media/platform/sunxi-vin/modules/sensor/$sensor_name*.c
	#find -name "$sensor_name*.c" $Top_Path/lichee/linux-4.9/drivers/media/platform/sunxi-vin/modules/sensor/

	if [ $? -eq 0 ] ;then
		echo "\nTarget_sensor's driver is in the kernel !\n"
	else
		echo "\nKernel have no driver for target_sensor , please add in it !\n"
		mv $Top_Path/$sensor_name.c  $Top_Path/lichee/linux-4.9/drivers/media/platform/sunxi-vin/modules/sensor/
		if [ $? -eq 0 ] ;then
	...
	grep "$sensor_name" $Top_Path/lichee/linux-4.9/drivers/media/platform/sunxi-vin/modules/sensor/Makefile
	if [ $? -eq 0 ] ;then
		echo "The file_path : lichee/linux-4.9/drivers/media/platform/sunxi-vin/modules/sensor/Makefile\n"
	else
		echo "obj-m                   += $sensor_name.o" >> $Top_Path/lichee/linux-4.9/drivers/media/platform/sunxi-vin/modules/sensor/Makefile
	...
	#  write modules.mk
	modules_mk_dir=$Top_Path/target/allwinner/$Target_Board/modules.mk
	sensor_module_1=" FILES+=\$(LINUX_DIR)/drivers/media/platform/sunxi-vin/modules/sensor/$sensor_name.ko"
	...
	#  write S00mpp
	S00mpp_dir=$Top_Path/target/allwinner/$Target_Board/busybox-init-base-files/etc/init.d/S00mpp
